...
func (uq *UserQuery) QueryAppointment() *SpecializedappointQuery {
	query := &SpecializedappointQuery{config: uq.config}
...
func (uq *UserQuery) QuerySpecializeddiag() *SpecializeddiagQuery {
	query := &SpecializeddiagQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithAppointment(opts ...func(*SpecializedappointQuery)) *UserQuery {
	query := &SpecializedappointQuery{config: uq.config}
...
func (uq *UserQuery) WithSpecializeddiag(opts ...func(*SpecializeddiagQuery)) *UserQuery {
	query := &SpecializeddiagQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withSpecializeddiag = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withSpecializeddiag; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.withFKs = true
		query.Where(predicate.Specializeddiag(func(s *sql.Selector) {
			s.Where(sql.InValues(user.SpecializeddiagColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
