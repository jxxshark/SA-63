...
func (pq *PatientQuery) QueryAppointment() *SpecializedappointQuery {
	query := &SpecializedappointQuery{config: pq.config}
...
func (pq *PatientQuery) WithAppointment(opts ...func(*SpecializedappointQuery)) *PatientQuery {
	query := &SpecializedappointQuery{config: pq.config}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withAppointment; query != nil {
		...
		for _, n := range neighbors {
			fk := n.patientid
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "patientid" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patientid" returned %v for node %v`, *fk, n.ID)
			}
